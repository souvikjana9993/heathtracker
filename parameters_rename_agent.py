...
def load_existing_mappings(filename):
    ...
    try:
        ...
        logging.info(
            f"Mapping file '{filename}' not found. Starting with empty mappings."
        )
        return {}
    except json.JSONDecodeError:
        logging.error(
            f"Invalid JSON in mapping file '{filename}'. Starting with empty mappings."
        )
        return {}
    return {}
...
def extract_range_values(reference_interval_str):
    ...
    try:
        ...
        match = re.match(
            r"([+-]?\d*\.?\d+)\s*-\s*([+-]?\d*\.?\d+)", reference_interval_str
        )
        if match:
    ...
    try:
        ...
        if json_string.startswith("{") and json_string.endswith("}"):
            ...
            try:
                ...
                logging.info(
                    "Successfully normalized new parameter names using Gemini."
                )
                ...
                logging.error(
                    f"Problematic JSON string: {json_string}"
        ...
        for p in data.get("parameters", []):
            ...
            if original_name in normalized_names and normalized_names[original_name]:
                ...
                if normalized_name != original_name:
                    ...
                    if (
                        original_name not in renamed_mapping
                        and normalized_name != original_name
                    ):
...
def fix_parameters_across_json(source_dir, dest_dir):
    ...
    renamed_mapping, renamed_counts = rename_parameters(
        all_parameters, normalized_names
    )
    ...
    save_renamed_mapping(
        normalized_names, RENAMED_MAPPING_FILE
    )  # Save all normalized names, including existing
